# dsar-toolkit pipeline orchestration — design (v1)

**Status:** v1 — 2026-05-22, drafted. Authoritative answer to "what
runs in what order, what runs in parallel, how do surgical re-runs
behave."

**Relationship to other specs.**
[`2026-05-22-zen-tei-integration-design-v3.md`](2026-05-22-zen-tei-integration-design-v3.md)
describes what each new module *is*. This document describes how
the orchestrator chains them — and the existing 4-stage pipeline —
...
|---|---|---|---|
| v1 | 2026-05-22 | this commit | Initial draft. Defines the data-dependency DAG; identifies parallel branches; specifies the orchestrator + per-module CLI dual-entry model; locks resume semantics via per-artefact `upstream_hash` fields; describes the LLM-rate-limit semaphore at the scope-classify gate. |
...
```
                ...
                │                                │
                ▼                                ▼
        embeddings.jsonl              raw entity tags
        (from raw text)               (from raw text via
                │                      spaCy + regex + CloakLLM)
                │                                │
        ┌───────┼───────┐                       │
        ▼       ▼       ▼                       │
   person_  cosine-     │                       │
...
`source → embed → cosine → rerank → LLM scope-classify → redact →
export`.

Parallel branches at each fan-out point:
- `embed` and `raw entity tags` both depend only on raw text →
  parallel.
- `person_index` (people_register's alias clustering) and
  `cosine-passing set` both depend only on `embeddings.jsonl` →
  parallel.
- Inside detect, the spaCy/regex/CloakLLM passes can interleave
  with embedding/rerank; they only need to sync at LLM scope-
  classify.

The **synchronisation barrier** is the LLM scope-classify step:
it needs BOTH the reranker output (or cosine-passing set, in
shadow mode) AND the raw entity tags. Nothing downstream of that
barrier can start until both arrive.
