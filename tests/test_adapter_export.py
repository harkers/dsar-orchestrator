...
Adapter shells out to ``python -m dsar_pipeline.export`` (bake is a
separate preceding stage); tests inject a fake runner so subprocess
never fires.
"""
...
def _export_runner(
    ...
    *,
    ...
    """Fake runner for the export module only.

    ``redacted/`` is pre-seeded by _seed_case (bake is a separate stage).
    The runner writes ``output/<ref>.pdf`` files mimicking packaging.
    """
    ...
    # Pre-seed the redacted tree so upstream_hash is non-empty.
...
def test_writes_output_manifest(tmp_path: Path) -> None:
    ...
    adapter.run_for_case(_make_cfg(case_path), runner=_export_runner(case_path))
...
def test_upstream_hash_depends_on_redacted_tree(tmp_path: Path) -> None:
    ...
    adapter.run_for_case(
        _make_cfg(case_path),
        runner=_export_runner(case_path, redacted_files=["d1.txt"]),
    )
    first = json.loads((case_path / "output" / "manifest.json").read_text())["upstream_hash"]

    adapter.run_for_case(
        _make_cfg(case_path),
        runner=_export_runner(case_path, redacted_files=["d1.txt", "d2.txt"]),
    )
    second = json.loads((case_path / "output" / "manifest.json").read_text())["upstream_hash"]
    assert first != second


def test_runner_called_once_for_export(tmp_path: Path) -> None:
    case_path = _seed_case(tmp_path)
    # Pre-seed redacted tree (bake is a separate stage).
    ...
    assert len(calls) == 1
    assert calls[0] == [sys.executable, "-m", "dsar_pipeline.export"]
...
def test_raises_when_export_fails(tmp_path: Path) -> None:
    case_path = _seed_case(tmp_path)
    # Redacted tree pre-exists (bake ran as a separate stage).
    (case_path / "redacted" / "d1.txt").write_text("x")
...
def test_raises_when_output_dir_missing(tmp_path: Path) -> None:
    ...
    def runner(argv, env, cwd):
        # export returns 0 but no output/ dir gets written
        return subprocess.CompletedProcess(args=argv, returncode=0)
