"""Conductor-owned redact-verify adapter — Stage 7.

Bridges to the toolkit's ``dsar_redact_verify.core.verify_case``. The
verifier inspects redacted output + emits per-doc verdicts to
``~/.dsar-audit/<case>/redact_verify.jsonl``; the conductor inspects
the Verdict object the call returns and halts the pipeline on any
failure.

This is a thin adapter: the toolkit already ships a Python entry, so
no subprocess is needed. The adapter adds testability (injectable
verifier) + a typed exception surface + a small no-op gate when the
operator has disabled verification.

**Retirement contract.** When the toolkit ships a thin Python entry
``dsar_redact_verify.core.verify_case_for_conductor(case_path)`` that
returns a typed dict matching the conductor's expectations, this
adapter retires.
"""
...
# Production resolution lazy-imports dsar_redact_verify.core.verify_case.
VerifyFn = Callable[[Any], Any]


def _default_verifier() -> VerifyFn:
    """Lazy-resolve ``dsar_redact_verify.core.verify_case``."""
    try:
        mod = importlib.import_module("dsar_redact_verify.core")
    except ImportError as exc:
        raise DSARPipelineError(
            "dsar_redact_verify.core is not installed. The conductor's "
            "redact-verify adapter needs it to validate redacted output. "
            "Install dsar-toolkit (pip install -e ~/projects/"
            "dsar-toolkit/) and retry."
        ) from exc

    def run(case_path: Any) -> Any:
        return mod.verify_case(case_path)
